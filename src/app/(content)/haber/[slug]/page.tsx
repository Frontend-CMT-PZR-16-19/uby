...
export default function NewsDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const fetchNewsPost = async () => {
    try {
      ...
      const relatedResponse = await fetch('/api/blog-posts?type=news&limit=2');
    ...
    <div className="min-h-screen bg-white">
      <article className="container mx-auto px-4 py-16">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-8 mb-16 pb-8 border-b-2 border-red-300">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-gray-700 font-medium text-sm uppercase tracking-wide" style={{ fontFamily: '"Helvetica Neue", Arial, sans-serif' }}>{post.category}</span>
            </div>
          </div>
                ...
                <div className="space-y-8">
                  {post.content.map((block: any, index: number) => {
                    if (block._type === 'block') {
                      ...
                      if (block.style === 'h2') {
                        return (
                          <h2 key={block._key || index} className="text-4xl font-bold text-gray-900 mb-6 mt-14 leading-tight border-b-2 border-red-300 pb-3" style={{ fontFamily: 'Georgia, "Times New Roman", serif' }}>
                            {block.children?.map((child: any) => child.text || '').join('') || ''}
                          </h2>
                        )
                      }
            ...
            <div className="max-w-6xl mx-auto">
              ...
              <div className="grid md:grid-cols-2 gap-8">
                {relatedPosts.map((relatedPost) => (
                  <Link 
                    key={relatedPost._id} 
                    href={`/haber/${typeof relatedPost.slug === 'string' ? relatedPost.slug : (relatedPost.slug as any)?.current || relatedPost._id}`}
                    className="group"
                  >
